...
BOOL InitD3D11(HINSTANCE hInstance) {
    ...
    {
        SimpleVertex(-0.5f, -0.5f, 0.5f), //bottom left
        SimpleVertex(-0.5f,  0.5f, 0.5f), //top left
        SimpleVertex(0.5f,  0.5f, 0.5f), //top right
        SimpleVertex(0.5f, -0.5f, 0.5f), //bottom right
    };
...
void UpdateScene() {
    ...
    ColorBuffer cbData;
    cbData.color = XMFLOAT4(red, green, blue, 1.0f);
    
    D3D11_MAPPED_SUBRESOURCE mappedResource;
    ZeroMemory(&mappedResource, sizeof(D3D11_MAPPED_SUBRESOURCE));
    deviceContext->Map(constantBuffer, 0, D3D11_MAP_WRITE_DISCARD, 0, &mappedResource);

    memcpy(mappedResource.pData, &cbData, sizeof(ColorBuffer));

...
ID3D11Buffer* cbPerObjectBuffer; //constant buffer to matrix transformations
...
cbPerObject constantBufferMatrix;
//--------------------------------------------------------------------------------------
// Structures
//--------------------------------------------------------------------------------------
struct SimpleVertex
...
BOOL InitD3D11(HINSTANCE hInstance) {
    ...
    hr = CompileShaderFromFile(L"Tutorial09.fxh", "VS", "vs_4_0", &pVSBlob);
    if (FAILED(hr))
    {
        MessageBox(nullptr,
    ...
    hr = CompileShaderFromFile(L"Tutorial09.fxh", "PS", "ps_4_0", &pPSBlob);
    if (FAILED(hr))
    {
        MessageBox(nullptr,
            L"The FX file cannot be compiled.  Please run this executable from the directory that contains the FX file.", L"Error", MB_OK);
    ...
    { \
        // Front Face
        SimpleVertex(-1.0f, -1.0f, -1.0f, 0.0f, 1.0f),
        SimpleVertex(-1.0f, 1.0f, -1.0f, 0.0f, 0.0f),
        SimpleVertex(1.0f, 1.0f, -1.0f, 1.0f, 0.0f),
        SimpleVertex(1.0f, -1.0f, -1.0f, 1.0f, 1.0f),
    ...
    translation = XMMatrixTranslation(0.5f, 0.0f, 0.0f);
    return TRUE;
...
void UpdateScene() {
    ...
    //Keep the square rotating
    rot += .00005f;
    if (rot > 6.26f) //2 PI radians = 360 degrees
        rot = 0.0f;
...
void RenderScene() {
    ...
    constantBufferMatrix.WVP = XMMatrixTranspose(WVP);
    deviceContext->UpdateSubresource(cbPerObjectBuffer, 0, NULL, &constantBufferMatrix, 0, 0);
